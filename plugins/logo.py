...
async def logosq(bot, message):
    ...
    try:
      ...
      if "graph.org" not in logo:
        return await x.edit("`❌ Something Went Wrong...`\n\nReport This Error In [SUPPORT GRP](t.me/tiyaan_bots_chat).")
      ...
      await message.reply_photo(logo,caption = "**🖼 Logo Generated By CC-MULTI-BOT**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
      await x.delete()
    except FloodWait:
      pass
...
async def logo(bot, message):
  ...
  if FSub == 400:
      return 
  try:
    text = message.text.replace("logo","").replace("/","").replace("@CC_MULTI_Bot","").strip().upper()
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By @CC_MULTI_Bot**")
  except FloodWait:
    pass
  except Exception as e:
    try:
